...
export function getProductUrl(
  ...
  const domain =
    (marketplaceId && MARKETPLACE_MAP[marketplaceId]?.domain) || "amazon.com";
...
export function extractProductFields(
  ...
  const summaries = Array.isArray(rawData.summaries)
    ? rawData.summaries[0] || {}
    : {};
  const attributesObj = Array.isArray(rawData.attributes)
    ? rawData.attributes[0] || {}
    : {};
  const dimensions = Array.isArray(rawData.dimensions)
    ? rawData.dimensions[0] || {}
    : {};
  const relationships = Array.isArray(rawData.relationships)
    ? rawData.relationships[0]?.relationships || []
    : [];

  const rawImages =
    Array.isArray(rawData.images) && rawData.images[0]?.images
      ? rawData.images[0].images
      : [];
  ...
  return {
    name:
      getNestedValue(summaries, "itemName") ||
      getNestedValue(attributesObj, "item_name") ||
      "",
    sku: (rawData.sku as string) || "",
    stockQuantity: (rawData.stockQuantity as number | string) || "",
    brand:
      getNestedValue(summaries, "brand") ||
      getNestedValue(attributesObj, "brand") ||
      (rawData["brand-name"] as string) ||
      "",
    color:
      getNestedValue(summaries, "color") ||
      getNestedValue(attributesObj, "color") ||
      "",
    partNumber:
      getNestedValue(summaries, "partNumber") ||
      getNestedValue(attributesObj, "part_number") ||
      (rawData.sku as string) ||
      "",
    manufacturer:
      getNestedValue(summaries, "manufacturer") ||
      getNestedValue(attributesObj, "manufacturer") ||
      "",
    description: getNestedValue(attributesObj, "product_description") || "",
    itemTypeKw: getNestedValue(attributesObj, "item_type_keyword") || "",
    category:
      (rawData.category as string) || summaries?.browseClassification?.displayName || "",
    price: (rawData.price as string) || "",
    itemCondition: (rawData["item-condition"] as string) || "New",
    pkgWeight:
      getDimensionValue(dimensions, "package") ||
      getDimensionValue(dimensions?.package, "weight") ||
      "",
    itemWeight:
      getDimensionValue(dimensions, "item") ||
      getDimensionValue(dimensions?.item, "weight") ||
      "",
    images,
    relationships,
  };
...
/**
 * Extract generic relationship pointers from Amazon SP-API item payload.
 * Provides child IDs and parent IDs if this is part of a variation family.
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function extractRelationships(rawData: Record<string, any>): {
  childIds: string[];
  parentId?: string;
} {
    ...
    for (const byMarketplace of relationships) {
      ...
      for (const rel of byMarketplace.relationships) {
        if (rel.type === "VARIATION") {
          if (Array.isArray(rel.childAsins)) {
            for (const childAsin of rel.childAsins) {
              if (childAsin) childAsinSet.add(childAsin);
            }
          }
          if (Array.isArray(rel.parentAsins) && rel.parentAsins.length > 0) {
            parentId = rel.parentAsins[0];
          }
        }
      }
    }
