...
- Order sync freshness (`channels.last_order_sync_at` and visible channel sync pills) must be advanced as soon as order ingestion succeeds, including inside channel handlers before any long-running reconciliation/continuation block. Do not delay the order-sync success marker behind finance reconciliation, stock continuation, shipped-order reconciliation, or any other optional/background work; those workflows may retry independently without making the channel look stale.
- Active-user/on-demand sync triggers must be scheduled through reliable request lifecycle work such as Next.js `after()`, not untracked fire-and-forget promises that can be dropped when a server component render completes.
