...
- Order sync freshness (`channels.last_order_sync_at` and visible channel sync pills) must be advanced as soon as order ingestion succeeds. Do not delay the order-sync success marker behind finance reconciliation, stock continuation, or any other optional/background work; those workflows may retry independently without making the channel look stale.
